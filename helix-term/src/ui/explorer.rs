...
impl Explorer {
    ...
    fn render_embed(
        ...
        .clip_bottom(1);
        let background = cx.editor.theme.get("ui.tree");
        ...
        let list_area = match position {
            ExplorerPosition::Left => {
                render_block(side_area.clip_left(1), surface, Borders::empty()).clip_bottom(1)
            }
            ExplorerPosition::Right => {
                render_block(side_area.clip_right(1), surface, Borders::empty()).clip_bottom(1)
            }
        };
